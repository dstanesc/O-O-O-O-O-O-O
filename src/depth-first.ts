...
async function traverseVertices(accessor: ElementAccessor, refs: Iterable<VertexRef>, visitor: TraversalVisitor) {
...
async function traverseEdges(accessor: ElementAccessor, refs: Iterable<EdgeRef>, vertexTrace: Set<VertexRef>, visitor: TraversalVisitor) {
    for (const ref of refs) {
        await traverseEdge(accessor, ref, vertexTrace, visitor)
    }
}

async function traverseVertex(accessor: ElementAccessor, ref: VertexRef, vertexTrace: Set<VertexRef>, visitor: TraversalVisitor) {
    if (ref !== undefined) {
        if (!vertexTrace.has(ref)) {
            ...
            if (vertex.nextEdge !== undefined)
                await traverseEdge(accessor, vertex.nextEdge, vertexTrace, visitor)
            if (visitor.startProp !== undefined && vertex.nextProp !== undefined)
                await traverseProp(accessor, vertex.nextProp, visitor)
            if (visitor.endVertex !== undefined)
                await visitor.endVertex(vertex)
        }
    }
}

async function traverseEdge(accessor: ElementAccessor, ref: EdgeRef, vertexTrace: Set<VertexRef>, visitor: TraversalVisitor) {
    if (ref !== undefined) {
        const edge: Edge = await accessor.getEdge(ref)
        if (visitor.startEdge !== undefined)
            await visitor.startEdge(edge)
        if (visitor.startProp !== undefined && edge.nextProp !== undefined)
            await traverseProp(accessor, edge.nextProp, visitor)
        if (edge.target !== undefined) // true
        ...
        if (visitor.endEdge !== undefined)
            await visitor.endEdge(edge)
    }
}

async function traverseProp(accessor: ElementAccessor, ref: PropRef, visitor: TraversalVisitor) {
    if (ref !== undefined) {
        const prop: Prop = await accessor.getProp(ref)
        if (visitor.startProp !== undefined)
            await visitor.startProp(prop)
        if (prop.nextProp !== undefined)
            await traverseProp(accessor, prop.nextProp, visitor)
        if (visitor.endProp !== undefined)
            await visitor.endProp(prop)
    }
}

export { TraversalVisitor, traverseVertices, traverseEdges, traverseVertex, traverseEdge, traverseProp }