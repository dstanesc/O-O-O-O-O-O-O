...
const vertexTest = async (size: number, updatePos: number, update: number, increment: number) => {
    const { baseline, current, updated, added } = await vertexCollection(size, updatePos, update, increment)
    const { root: baseRoot, index: baseIndex, blocks: baseBlocks } = baseline
    const { root: currentRoot, index: currentIndex, blocks: currentBlocks } = current
    const { baselineChangesRecords } = deltaFactory({ linkCodec, blockCodec })
    const recordsDecode = async (bytes: Uint8Array) => new VertexDecoder(bytes).read()
    const { added: addedFound, updated: updatedFound } = await baselineChangesRecords({ recordSize: OFFSET_INCREMENTS.VERTEX_INCREMENT, baseRoot, baseIndex, baseStore: blockStore, currentRoot, currentIndex, currentStore: blockStore, recordsDecode })
...
const edgeTest = async (size: number, updatePos: number, update: number, increment: number) => {
    const { baseline, current, updated, added } = await edgeCollection(size, updatePos, update, increment)
    const { root: baseRoot, index: baseIndex, blocks: baseBlocks } = baseline
    const { root: currentRoot, index: currentIndex, blocks: currentBlocks } = current
    const { baselineChangesRecords } = deltaFactory({ linkCodec, blockCodec })
    const recordsDecode = async (bytes: Uint8Array) => new EdgeDecoder(bytes).read()
    const { added: addedFound, updated: updatedFound } = await baselineChangesRecords({ recordSize: OFFSET_INCREMENTS.EDGE_INCREMENT, baseRoot, baseIndex, baseStore: blockStore, currentRoot, currentIndex, currentStore: blockStore, recordsDecode })
...
const propTest = async (size: number, updatePos: number, update: number, increment: number) => {
    const { baseline, current, updated, added } = await propCollection(size, updatePos, update, increment)
    const { root: baseRoot, index: baseIndex, blocks: baseBlocks } = baseline
    const { root: currentRoot, index: currentIndex, blocks: currentBlocks } = current
    const { baselineChangesRecords } = deltaFactory({ linkCodec, blockCodec })
    const recordsDecode = async (bytes: Uint8Array) => new PropDecoder(bytes, linkCodec.decode, blockCodec.decode, blockStore.get).read()
    const { added: addedFound, updated: updatedFound } = await baselineChangesRecords({ recordSize: OFFSET_INCREMENTS.PROP_INCREMENT, baseRoot, baseIndex, baseStore: blockStore, currentRoot, currentIndex, currentStore: blockStore, recordsDecode })
...
const vertexCollection = async (size: number, updatePos: number, update: number, increment: number) => {
    const vertices = []
    for (let i = 0; i < size; i++) {
        const vertex: Vertex = { status: Status.CREATED, offset: i * OFFSET_INCREMENTS.VERTEX_INCREMENT, type: 1 }
    ...
    for (let i = 0; i < update; i++) {
        ...
        const vertex: Vertex = { status: Status.UPDATED, offset: pos * OFFSET_INCREMENTS.VERTEX_INCREMENT + i * OFFSET_INCREMENTS.VERTEX_INCREMENT, type: 999 }
        vertices[pos + i] = vertex
        updated.push(vertex)
    }
    const added = []
    for (let i = 0; i < increment; i++) {
        const vertex: Vertex = { status: Status.CREATED, offset: size * OFFSET_INCREMENTS.VERTEX_INCREMENT + i * OFFSET_INCREMENTS.VERTEX_INCREMENT, type: 777 }
        vertices.push(vertex)
        added.push(vertex)
    }
...
const edgeCollection = async (size: number, updatePos: number, update: number, increment: number) => {
    const edges = []
    for (let i = 0; i < size; i++) {
        const edge: Edge = { status: Status.CREATED, offset: i * OFFSET_INCREMENTS.EDGE_INCREMENT, type: 1, source: 100 + i, target: 101 + i }
        edges.push(edge)
    }
    ...
    for (let i = 0; i < update; i++) {
        const pos = updatePos //Math.floor(size / 3)
        const edge: Edge = { status: Status.UPDATED, offset: pos * OFFSET_INCREMENTS.EDGE_INCREMENT + i * OFFSET_INCREMENTS.EDGE_INCREMENT, type: 999, source: 888, target: 3333 }
        edges[pos + i] = edge
        updated.push(edge)
    }
    const added = []
    for (let i = 0; i < increment; i++) {
        const edge: Edge = { status: Status.CREATED, offset: size * OFFSET_INCREMENTS.EDGE_INCREMENT + i * OFFSET_INCREMENTS.EDGE_INCREMENT, type: 777, source: 888, target: 3333 }
        edges.push(edge)
        added.push(edge)
    }
...
const propCollection = async (size: number, updatePos: number, update: number, increment: number) => {
    const props = []
    for (let i = 0; i < size; i++) {
        const prop: Prop = { status: Status.CREATED, offset: i * OFFSET_INCREMENTS.PROP_INCREMENT, type: 1, key: 1, value: '123' }
        props.push(prop)
    }
    ...
    for (let i = 0; i < update; i++) {
        const pos = updatePos //Math.floor(size / 3)
        const prop: Prop = { status: Status.UPDATED, offset: pos * OFFSET_INCREMENTS.PROP_INCREMENT + i * OFFSET_INCREMENTS.PROP_INCREMENT, type: 999, key: 2, value: '123' }
        props[pos + i] = prop
        updated.push(prop)
    }
    const added = []
    for (let i = 0; i < increment; i++) {
        const prop: Prop = { status: Status.CREATED, offset: size * OFFSET_INCREMENTS.PROP_INCREMENT + i * OFFSET_INCREMENTS.PROP_INCREMENT, type: 777, key: 2, value: '123' }
        props.push(prop)
        added.push(prop)
    }
...
const verticesCreate = async (array: Vertex[]) => {
    ...
    const { root, index, blocks } = await create({ buf, chunk, encode: linkCodec.encode })
...
const propsCreate = async (array: Prop[]) => {
    const buf = await new PropEncoder(array, blockCodec.encode, blockStore.put).write()
    const { root, index, blocks } = await create({ buf, chunk, encode: linkCodec.encode })
    for (const block of blocks) await blockStore.put(block)
    return { root, index, blocks }
}