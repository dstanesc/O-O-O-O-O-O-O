...
describe('Api', function () {
    ...
    test("internal, minimal creation, proto-schema aware, deterministic root and offsets", async () => {
        ...
        const store = graphStore({ chunk, linkCodec, blockCodec: multiBlockCodec, blockStore })
        ...
        assert.equal('bafkreiguifcsbkfb7jlxr7inlp5fqukmve7mv2po234jj73pp7nryvbcxu', root.toString())
    })
    ...
    test("equivalent proto-gremlin, minimal creation, proto-schema aware, deterministic root and offsets", async () => {
        ...
        const gf: ProtoGremlinFactory = protoGremlinFactory({ chunk, linkCodec, blockCodec: multiBlockCodec, blockStore, rootStore })
        ...
        const v2 = await tx.addV(ObjectTypes.TWEET)
            .property(KeyTypes.JSON, { hello: "v2" }, PropTypes.COMMENT)
            .property(KeyTypes.JSON, { hello: "v3" }, PropTypes.COMMENT).next()
    ...
    test("internal minimal creation, proto-schema aware w/ additional edge properties", async () => {
        ...
        assert.equal('bafkreicklvs2aaeqfvs6f2pgcki2gont35chka2loq7mlah7yu4tj6bsvy', root.toString())
    })
...
async function createSchemaAwareGraph() {
    ...
    const {
        vertexRoot, vertexOffset, vertexIndex, edgeRoot, edgeOffset, edgeIndex, propRoot, propOffset, propIndex
    } = index
